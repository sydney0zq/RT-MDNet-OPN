...
def damage_masks(labels, shift=True, scale=True, rotate=True, dilate=True):
  ...
  Args:
    labels: Int32 labels tensor of shape (height, width).
  ...
  Returns:
    The damaged version of labels. (1, 1, height, width)
  """
  damaged_masks = damage_masks_np(labels, shift, scale, rotate, dilate)
  damaged_masks = damaged_masks[None, None, ...]
...
def _damage_single_object_mask(mask, shift, scale, rotate, dilate):
  ...
  Args:
    mask: Boolean numpy array of shape(height, width).
    shift: Boolean, whether to damage the masks by shifting.
    scale: Boolean, whether to damage the masks by scaling.
    rotate: Boolean, whether to damage the masks by rotation.
    dilate: Boolean, whether to damage the masks by dilation.
...
def _shift_mask(mask, max_shift_factor=0.2):
  ...
  if len(nzy) == 0 or len(nzx) == 0: return mask
  h = nzy.max() - nzy.min()
  w = nzx.max() - nzx.min()
  size = np.sqrt(h * w)
  offset = np.random.uniform(-size * max_shift_factor, size * max_shift_factor,
                             2)
  shifted_mask = interpolation.shift(mask, offset, order=0).astype('bool')
  return shifted_mask
...
def _rotate_mask(mask, max_rot_degrees=5.0):
...
def _dilate_mask(mask, dilation_radius=10):
  ...
  dilated_mask = morphology.binary_dilation(mask, selem=disk)
  return dilated_mask
...
if __name__ == "__main__":
    ...
    #mask[10:100, 300: 390, 0] = 255
    #mask[200:290, 260: 380, 0] = 255
    #cv2.imwrite("mask.jpg", mask)
    #dmask = damage_masks(mask)
    #dmask = _scale_mask(mask)
    #dmask = _shift_mask(mask)
    #dmask = _dilate_mask(mask)
    dmask = damage_masks(mask)
    import pdb
    pdb.set_trace()
    #cv2.imwrite("dmask.jpg", dmask)
